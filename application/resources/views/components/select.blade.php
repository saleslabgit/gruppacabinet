...
<select data-custom-select id="{{ $name }}" name="{{ $name }}" @required($required) aria-invalid="{{ $error ? 'true' : 'false' }}" aria-describedby="{{ $name }}-help{{ $error ? ' '.$name.'-error' : '' }}" {{ $attributes->class(['form-select', 'is-invalid' => (bool) $error]) }}>
