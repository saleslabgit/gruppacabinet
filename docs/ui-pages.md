...
- `/cabinet/groups/{id}`: данные, текущий комментарий/причина, история,
  разрешённые действия и явная недоступность заявок.
...
- `/cabinet/admin/groups`: поиск, тариф/статус, сортировка, быстрые фильтры
  approved/abandoned; нет фильтра успешного платежа.
...
Настройки, Выход. Заявки пока не подключены. Существующие prototype payment
